...
module.exports = {
    name: 'unwhitelist',
    description: 'Remueve a un jugador de la lista blanca del servidor.',
    usage: '`'+prefix+'unwhitelist {nombre|ID|dirección del servidor} {jugador de Minecraft}`',
    permission: '`ADMINISTRADOR`',
    execute(bot, msg, args){
        async function APIunwhitelist(){
            try{
                ...
                const playerWhitelistRemovedEmbed = new Discord.MessageEmbed()
                    .setDescription(`El jugador **${username}** has sido removido de la lista blanca del servidor **${server.name}**.`)
                    .setColor(config.embedColor) 
                await msg.channel.send({embeds:[playerWhitelistRemovedEmbed]})
            } catch(e){
                console.log(msg.content + ' | Usuario: ' + msg.author.tag)
                console.error(`Error al usar el comando "unwhitelist": ${e.message}`)
